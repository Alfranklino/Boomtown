...
const BORROW_ITEM = gql`
  mutation borrowItem($borrowInfo: BorrowItemInput!) {
    borrowItem(borrowInfo: $borrowInfo) {
      name
      status
      borrower_id
      id
...
class BorrowItem extends Component {
  ...
  render() {
    return (
      <Fragment>
        <h1>Here is where to Borrow an item from the list</h1>

        <Mutation mutation={BORROW_ITEM}>
            ...
            return (
              <Formik
                ...
                onSubmit={(values, { setSubmitting }) => {
                  ...
                  borrowItem({ variables: { borrowInfo: values } });
                  setSubmitting(false);
                ...
                {({ errors, status, touched, values, handleChange, handleBlur, handleSubmit }) => (
                  <div className='container'>
                    <form className='inputsFields' onSubmit={handleSubmit}>
                      ...
                      <h3>The following product is borrowed: {this.state.id}. Congratulations!.</h3>
                    </form>
                  </div>
                )}
              </Formik>
